...
test("getNode returns null for non-existent path", () => {
  const fs = new VirtualFileSystem();
  expect(fs.getNode("/nonexistent")).toBeNull();
});

test("getAllFiles returns empty map when no files exist", () => {
  const fs = new VirtualFileSystem();
  const files = fs.getAllFiles();
  expect(files.size).toBe(0);
});

test("reset clears all files and restores initial state", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");
  fs.createDirectory("/src");
  fs.createFile("/src/index.ts", "export {}");
